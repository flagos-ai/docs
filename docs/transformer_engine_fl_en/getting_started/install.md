...
This guide covers installing TransformerEngine-FL starting from a pre-built Docker image, and validating the installation with an end-to-end Qwen3 training job. Installation starts from the Docker image; the component packages are then installed inside the container.

The workflow uses the following FlagOS components together:

- **TransformerEngine-FL** — the transformer operator layer (this component).
- **Megatron-LM-FL** — the training engine. See [Megatron-LM-FL](https://github.com/flagos-ai/Megatron-LM-FL).
...
## 3. Get the image and start the container
...
The validated tag differs slightly per platform. Use the tag matching your platform:
...
### 5.1 Install FlagScale

```bash
cd /workspace/FlagScale
git checkout 2.1.0
pip install .                     # add --no-build-isolation on T-Head PPU
...
megatron-core                            0.18.2+e15cb6928   # 0.18.2+c8fa61f2e on T-Head PPU
megatron-energon                         6.0.1
```

### 5.3 Install TransformerEngine-FL
...
On NVIDIA, build against the CUDA toolkit instead:
...
On T-Head PPU and Ascend, the same package is reported as `transformer_engine 2.17.0+69cf722e` with `transformers 5.8.1`.

---

## 6. Prepare the dataset and tokenizer
...
## 7. Configure and run the training
...
## 8. Switch to the FlagOS compiler and operator library
...
**`ValueError: Invalid entry in hostfile`** — caused by `flag_gems_log_path` pointing at the hostfile, as described in section 8.3.
...
**FlagTree and FlagGems cannot be compiled on `/workspace`** — see the note in section 8.1. Build from local disk.
