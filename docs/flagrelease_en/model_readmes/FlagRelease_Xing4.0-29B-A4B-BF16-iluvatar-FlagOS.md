...
# Evaluation Results
## Benchmark Result
| Metrics      | Xing4.0-29B-A4B-Nvidia-Origin | Xing4.0-29B-A4B-Iluvatar-FlagOS |
|--------------|--------------------------------|--------------------------------------|
...
modelscope download --model FlagRelease/Xing4.0-29B-A4B-BF16-iluvatar-FlagOS --local_dir /data/Xing4.0-29B-A4B
```

### Start the Container
```bash
docker run -itd --name flagos --privileged --network=host -v /dev:/dev -v /lib/modules:/lib/modules -v /usr/local/corex/bin/ixsmi:/usr/local/bin/ixsmi:ro -v /data:/data harbor.baai.ac.cn/flagrelease-public/iluvatar-corex4.5.0-flagtree0.6.0-triton3.6.0-cxnone-vllm_fl0.24.0:2026082-xingchen4-0907 sleep infinity
```
### Start the Server
```bash
...
vllm serve /data/Xing4.0-29B-A4B \
  --served-model-name xingchen4-0907 \
  --dtype bfloat16 \
  ...
  --chat-template /models/XingChen4-29B-A4B-0907/chat_template.jinja \
  --enforce-eager \
  --trust-remote-code
```
