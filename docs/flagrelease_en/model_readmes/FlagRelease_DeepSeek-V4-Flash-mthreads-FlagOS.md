...
# Evaluation Results
## Benchmark Result
| Metrics      | DeepSeek-V4-Flash-Nvidia-Origin | DeepSeek-V4-Flash-Mthreads-FlagOS |
|--------------|--------------------------------|--------------------------------------|
| GPQA | 0.697                              | 0.6566                                   |
| Aime       | 0.7333                              | 0.6667                                    |
...
```
docker pull harbor.baai.ac.cn/flagrelease-public/flagrelease-mthreads-deepseek:202604240000
```

### Download Open-source Model Weights
```bash
pip install modelscope
modelscope download --model FlagRelease/DeepSeek-V4-Flash-mthreads-FlagOS --local_dir /data/DeepSeek-V4-Flash-mthreads-FlagOS
```

### Start the Container
```bash
docker run -itd --privileged --net host --name=flagos -w /workspace -v /data:/data  --env MTHREADS_VISIBLE_DEVICES=all --shm-size=80g  harbor.baai.ac.cn/flagrelease-public/flagrelease-mthreads-deepseek:202604240000 /bin/bash
```
...
```bash
cd /workspace/code/
torchrun --nproc-per-node=8 generate.py --ckpt-path /path/to/your/ckpt --config config.json --input-dir /workspace/code/cases --output-csv results.csv
