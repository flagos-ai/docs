...
# Evaluation Results
## Benchmark Result
| Metrics      | DeepSeek-V4-Flash-Nvidia-Origin | DeepSeek-V4-Flash-Iluvatar-FlagOS |
|--------------|--------------------------------|--------------------------------------|
| GPQA | 0.697                              | 0.6333                                    |
| Aime       | 0.7333                              | 0.7333                                    |
...
```
docker pull harbor.baai.ac.cn/flagrelease-public/flagrelease-iluvatar-deepseek:202604240000
```

### Download Open-source Model Weights
```bash
pip install modelscope
modelscope download --model FlagRelease/DeepSeek-V4-Flash-iluvatar-FlagOS --local_dir /data/DeepSeek-V4-Flash-iluvatar-FlagOS
```

### Start the Container
```bash
docker run --name flagos -dit -v /usr/src:/usr/src -v /data:/data -v /mnt/share:/mnt/share -v /lib/modules:/lib/modules -v /dev:/dev --privileged --cap-add=ALL --pid=host --net=host harbor.baai.ac.cn/flagrelease-public/flagrelease-iluvatar-deepseek:202604240000
```
...
```bash
cd /root/hello0423
# 修改 run_node_batch.sh 里面的模型地址 为真实地址
./run_node_batch.sh \
  --prompt_dir 3case \
  --output_csv output2.csv \
  --max_tokens 2048 \
  --master_addr 10.31.30.121 \
  --node_rank 0 \
  --master_port 29500
