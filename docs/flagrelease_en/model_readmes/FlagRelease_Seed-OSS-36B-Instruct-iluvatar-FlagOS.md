...
# Evaluation Results
## Benchmark Result
| Metrics             | Seed-OSS-36B-Instruct-Nvidia-Origin | Seed-OSS-36B-Instruct-Iluvatar-FlagOS |
|---------------------|-------------------------------------|---------------------------------------|
...
modelscope download --model FlagRelease/Seed-OSS-36B-Instruct-iluvatar-FlagOS --local_dir /data/Seed-OSS-36B-Instruct-iluvatar-FlagOS
```
...
docker run --name seed-oss-36b-flagos -itd \
  --shm-size="32g" \
  -v /usr/src:/usr/src \
  -v /data/Seed-OSS-36B-Instruct-iluvatar-FlagOS:/data/Seed-OSS-36B-Instruct-iluvatar-FlagOS \
  -v /dev:/dev \
  --privileged \
  --cap-add=ALL \
  --pid=host \
...
nohup vllm serve --model /data/Seed-OSS-36B-Instruct-iluvatar-FlagOS --served-model-name seed-oss-36b-flagos --port 8387 --tensor-parallel-size 4 --trust-remote-code --max-model-len 30000 --enforce-eager >eager-seed-oss-gems.log 2>&1 &
```

## Service Invocation
### Invocation Script
```bash
curl http://localhost:8387/v1/chat/completions \
