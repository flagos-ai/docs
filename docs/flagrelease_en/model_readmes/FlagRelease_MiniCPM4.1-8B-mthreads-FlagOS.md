...
# Evaluation Results
## Benchmark Result
| Metrics      | MiniCPM4.1-8B-mthreads-FlagOS-Origin | MiniCPM4.1-8B-mthreads-FlagOS-FlagOS |
|--------------|--------------------------------------|--------------------------------------|
| GPQA_Diamond | 54.0 | 50.0 |
| ERQA | - | - |
| Aime24 | - | - |
...
```bash
docker pull harbor.baai.ac.cn/flagrelease-project/minicpm4.1-8b-mthreads001-gems5.3.0-tree0.6.1-cxnone-plugin0.2.0-vllm0.20.2-cp310-pt27-musa43-x64-3.3.6-server:202607261137-v3
```

### Download Open-source Model Weights
```bash
pip install modelscope
modelscope download --model FlagRelease/MiniCPM4.1-8B-mthreads-FlagOS --local_dir /data/MiniCPM4.1-8B-FlagOS
```

### Start the Container
```bash
docker run -d --name flagos --net=host --ipc=host --privileged --shm-size=16g --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --tmpfs /tmp:exec -e MTHREADS_VISIBLE_DEVICES=all -e MTHREADS_DRIVER_CAPABILITIES=all -v /data:/data harbor.baai.ac.cn/flagrelease-project/minicpm4.1-8b-mthreads001-gems5.3.0-tree0.6.1-cxnone-plugin0.2.0-vllm0.20.2-cp310-pt27-musa43-x64-3.3.6-server:202607261137-v3 sleep infinity
```
### Start the Server
```bash
vllm serve /data/MiniCPM4.1-8B-FlagOS --host 0.0.0.0 --port 8000 --served-model-name MiniCPM4.1-8B --tensor-parallel-size 1 --max-model-len 32768 --enforce-eager --trust-remote-code
```
...
# License
The model weights are derived from openbmb/MiniCPM4.1-8B and are open‑sourced under the Apache License 2.0: https://www.apache.org/licenses/LICENSE-2.0.txt
