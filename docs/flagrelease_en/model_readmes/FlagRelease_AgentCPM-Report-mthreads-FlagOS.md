...
# Evaluation Results
## Benchmark Result
| Metrics      | AgentCPM-Report-mthreads-FlagOS-Origin | AgentCPM-Report-mthreads-FlagOS-FlagOS |
|--------------|----------------------------------------|----------------------------------------|
| GPQA_Diamond | 34.0 | 54.0 |
| ERQA | - | - |
| Aime24 | - | - |
...
```bash
docker pull harbor.baai.ac.cn/flagrelease-project/agentcpm-report-mthreads001-gems5.3.0-tree0.6.1-cxnone-plugin0.2.0-vllm0.20.2-cp310-pt27-musa43-x64-3.3.6-server:202607260450-v3
```

### Download Open-source Model Weights
```bash
pip install modelscope
modelscope download --model FlagRelease/AgentCPM-Report-mthreads-FlagOS --local_dir /data/AgentCPM-Report-FlagOS
```

### Start the Container
```bash
docker run -d --name flagos --net=host --ipc=host --privileged --shm-size=16g --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --tmpfs /tmp:exec -e MTHREADS_VISIBLE_DEVICES=all -e MTHREADS_DRIVER_CAPABILITIES=all -v /data:/data harbor.baai.ac.cn/flagrelease-project/agentcpm-report-mthreads001-gems5.3.0-tree0.6.1-cxnone-plugin0.2.0-vllm0.20.2-cp310-pt27-musa43-x64-3.3.6-server:202607260450-v3 sleep infinity
```
### Start the Server
```bash
vllm serve /data/AgentCPM-Report-FlagOS \
--host 0.0.0.0 --port 8000 \
--tensor-parallel-size 1 \
--served-model-name AgentCPM-Report \
--trust-remote-code \
--max-model-len 32768
```
