...
# Evaluation Results
## Benchmark Result
| Metrics      | DeepSeek-V4-Pro-Nvidia-Origin | DeepSeek-V4-Pro-Nvidia-FlagOS |
|--------------|--------------------------------|--------------------------------------|
| GPQA | -                              | -                                    |
| Aime       | -                              | -                                    |
...
```bash
docker pull harbor.baai.ac.cn/flagrelease-public/flagrelease-nvidia-deekseek-pro:202604271111
```

### Download Open-source Model Weights
```bash
pip install modelscope
modelscope download --model FlagRelease/DeepSeek-V4-Pro-nvidia-FlagOS --local_dir /data/DeepSeek-V4-Pro-nvidia-FlagOS
```

### Start the Container
```bash
docker run --rm --init --detach --net=host --userns=host --ipc=host   --security-opt=seccomp=unconfined --privileged=true   --ulimit stack=67108864 --ulimit memlock=-1 --ulimit nofile=1048576:1048576   --shm-size=32G -v /data:/data --gpus all   -e NVIDIA_DRIVER_CAPABILITIES=compute,utility   --name flagos harbor.baai.ac.cn/flagrelease-public/flagrelease-nvidia-deekseek-pro:202604271111 sleep infinity
...
torchrun \
  ...
  --nproc_per_node=8 \
  --node_rank=0 \
  --master_addr <node1_ip> --master_port 39500 \
    generate.py --ckpt-path /data/DeepSeek-V4-Pro/ --config /data/DeepSeek-V4-Pro/code/config_1T573B_v4_lms.json --input-file prompt.txt --max-new-tokens 100
