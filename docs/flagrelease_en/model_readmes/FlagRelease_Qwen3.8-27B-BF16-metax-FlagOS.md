...
# Evaluation Results
## Benchmark Result
| Metrics | Qwen3.8-27B-Nvidia-Origin | Qwen3.8-27B-Metax-FlagOS |
|--------------|---------------------------|--------------------------|
| musr_murder_mysteries| 76.8                      | 75.6                     |
| GPQA_Diamond | 90.4                      | 89.34                    |
...
```bash
docker pull harbor.baai.ac.cn/flagrelease-public/qwen3.8-27b-metax001-gems5.4.0-tree0.5.1-cxnone-plugin0.2.0-vllm0.20.2-cp312-pt28-maca37-x64-3.8.1:202608140155
```

### Download Open-source Model Weights
```bash
pip install modelscope
modelscope download --model FlagRelease/Qwen3.8-27B-BF16-metax-FlagOS --local_dir /data/Qwen3.8-27B
```

### Start the Container
```bash
docker run -d \
  --name qwen38_27b_vllm \
  --network host \
  --shm-size 64g \
  --device /dev/dri:/dev/dri:rwm \
  --device /dev/mxcd:/dev/mxcd:rwm \
  -v /data:/data \
  harbor.baai.ac.cn/flagrelease-public/qwen3.8-27b-metax001-gems5.4.0-tree0.5.1-cxnone-plugin0.2.0-vllm0.20.2-cp312-pt28-maca37-x64-3.8.1:202608140155 \
  sleep infinity
docker exec -it qwen38_27b_vllm /bin/bash
```
### Start the Server
```bash
# 性能优化版
...
PROT=8066
VLLM_USE_MODELSCOPE=true vllm serve  ${MODEL} \
...
### Invocation Script
```bash
curl http://${your_ip}:8066/v1/chat/completions \
