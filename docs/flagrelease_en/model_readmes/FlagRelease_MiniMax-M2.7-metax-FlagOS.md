...
# Evaluation Results
## Benchmark Result
|Metrics|MiniMax-M2.7-Nvidia-Origin|MiniMax-M2.7-Metax-FlagOS|
|-------|---------------|---------------|
|Gpqa_Diamond |0.7071 |0.6717 |
|Aime24  |0.9  | 0.9 |
...
```bash
docker pull harbor.baai.ac.cn/flagrelease-public/flagrelease-metax-minimax:202604112246
```

### Download Open-source Model Weights
```bash
pip install modelscope
modelscope download --model FlagRelease/MiniMax-M2.7-metax-FlagOS --local_dir /data/MiniMax-M2.7
```
...
docker run -d \
  ...
  -v /data:/data \
  harbor.baai.ac.cn/flagrelease-public/flagrelease-metax-minimax:202604112246 \
...
vllm serve /data/MiniMax-M2.7 \
  ...
  --nnodes 2 \
  --node-rank 0 \
  --master-addr 192.168.8.20 \
  --port 9010 \
  --trust-remote-code

# node2
vllm serve /data/MiniMax-M2.5-New-bf16 \
  --tensor-parallel-size 8 \
  --pipeline-parallel-size 2 \
  --served-model-name minimax \
  --nnodes 2 \
...
### Invocation Script
```bash
curl http://0.0.0.0:9010/v1/chat/completions \
