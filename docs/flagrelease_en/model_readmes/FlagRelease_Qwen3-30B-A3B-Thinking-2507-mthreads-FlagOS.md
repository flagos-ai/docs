...
# Evaluation Results
## Benchmark Result
| Metrics      | Qwen3-30B-A3B-Thinking-2507-mthreads-FlagOS-Origin | Qwen3-30B-A3B-Thinking-2507-mthreads-FlagOS-FlagOS |
|--------------|----------------------------------------------------|----------------------------------------------------|
| GPQA_Diamond | 0 | 74.0 |
| ERQA | - | - |
| Aime24 | - | - |
...
```bash
docker pull harbor.baai.ac.cn/flagrelease-public/qwen3-30b-a3b-thinking-2507-mthreads001-gems5.3.0-tree0.6.1-cxnone-plugin0.2.0-vllm0.20.2-cp310-pt27-musa43-x64-3.3.6-server:202608031418-v2
```

### Download Open-source Model Weights
```bash
pip install modelscope
modelscope download --model FlagRelease/Qwen3-30B-A3B-Thinking-2507-mthreads-FlagOS --local_dir /data/Qwen3-30B-A3B-Thinking-2507-FlagOS
```

### Start the Container
```bash
docker run -d --name flagos --net=host --ipc=host --privileged --shm-size=16g --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --tmpfs /tmp:exec -e MTHREADS_VISIBLE_DEVICES=all -e MTHREADS_DRIVER_CAPABILITIES=all -v /data:/data harbor.baai.ac.cn/flagrelease-public/qwen3-30b-a3b-thinking-2507-mthreads001-gems5.3.0-tree0.6.1-cxnone-plugin0.2.0-vllm0.20.2-cp310-pt27-musa43-x64-3.3.6-server:202608031418-v2 sleep infinity
```
### Start the Server
```bash
VLLM_PLUGINS=fl vllm serve /data/Qwen3-30B-A3B-Thinking-2507-FlagOS --host 0.0.0.0 --port 8000 --served-model-name Qwen3-30B-A3B-Thinking-2507 --tensor-parallel-size 1 --max-model-len 32768 --trust-remote-code --enforce-eager
```
