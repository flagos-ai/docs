...
This guide covers installing Megatron-LM-FL starting from a pre-built Docker image, and validating the installation with an end-to-end Qwen3 training job. Installation starts from the Docker image; the component packages are then installed inside the container.

The workflow uses the following FlagOS components together:
...
- **FlagTree** / **FlagGems** (optional) — the FlagOS compiler and Triton-based operator library.

---

## 1. Supported platforms

End-to-end validation has been completed on MetaX, Hygon, Ascend, and T-Head PPU.
...
```{note}
T-Head PPU keeps using the `CUDA_VISIBLE_DEVICES` variable name; there is no separate `PPU_VISIBLE_DEVICES`.
Hygon requires `source /opt/dtk/env.sh` before any install or run command.
```

---

## 2. Get the image and start the container
...
Before installing the packages below, check inside the container that your device is visible (section 1 lists the command for each platform).

On CUDA, activate the environment and install FlashAttention after entering the container:
...
The tags validated on each platform (FlagTree tracks a different Triton version on Ascend):

| Component | MetaX | Hygon | Ascend | T-Head PPU |
|-----------|-------|-------|--------|------------|
...
# Experiment logging and async data loading
pip install wandb aiohttp
```

### 4.3 Install TransformerEngine-FL
...
```{note}
`TE_FL_SKIP_CUDA=1` is mandatory on non-NVIDIA platforms — without it the build tries to compile the CUDA kernels and fails. On NVIDIA, omit this variable and build against the CUDA toolkit.
```

---

## 5. Prepare the dataset and tokenizer
...
## 6. Configure and run the training
...
Edit `./examples/qwen3/conf/train.yaml` so that the selected model config matches the file name, and set the platform visible-devices variable:
...
## 7. Switch to the FlagOS compiler and operator library
...
git checkout 0.7.0+triton3.6    # 0.7.0+triton3.5 on Ascend
...
```{note}
On T-Head PPU, `/workspace` is an OSS object-store FUSE mount (`ossfs2`) that cannot host in-place ELF writes (the assembler fails with `file truncated`). Copy the sources — including `.git`, which `setuptools_scm` needs for the version number — to local disk (for example `/root/FlagTree`) and build there. The same applies to FlagGems.
```

### 7.2 Install FlagGems
...
### 7.3 Enable the FlagOS stack in training
...
A healthy 8-card run on T-Head PPU with the FlagOS operator stack enabled (`global_batch_size: 8`):
...
**`ValueError: Invalid entry in hostfile`** — caused by `flag_gems_log_path` pointing at the hostfile, as described in section 7.3.
...
**FlagTree and FlagGems cannot be compiled on `/workspace`** — see the note in section 7.1. Build from local disk.
...
- Use the platform-specific visible-devices variable from section 6.2 — an incorrect variable silently leaves the job on a single device.
