...
本指南介绍如何从预构建 Docker 镜像开始安装 Megatron-LM-FL，并通过一次端到端 Qwen3 训练任务验证安装。安装从 Docker 镜像开始，随后在容器内安装各组件包。
...
在开始下面的安装步骤前，先在容器内确认设备可见（各平台的检查命令见第 1 节）。
...
```{note}
非 NVIDIA 平台必须加 `TE_FL_SKIP_CUDA=1`——不加会尝试编译 CUDA kernel 并失败。NVIDIA 平台不加该变量，按 CUDA 工具链构建。
```
...
**`ValueError: Invalid entry in hostfile`** —— 由 `flag_gems_log_path` 指向 hostfile 引起，见 7.3 节。
...
**FlagGems attention 反向 kernel 报 `MLIRCompilationError: ub overflow`** —— attention 反向 kernel 所需的 unified buffer 超出 NPU 可用容量。可降低 `seq_length`（如从 4096 降到 1024），或改用原生 TransformerEngine 后端而不使用 FlagGems。编译器默认开启多缓冲（`--enable-auto-multi-buffer=True`），会使需求翻倍；设置 `num_stages=1` 可关闭。

### 9.4 平头哥 PPU

**FlagTree 与 FlagGems 无法在 `/workspace` 编译** —— 见 7.1 节说明，请在本地磁盘编译。
...
- 请根据平台镜像切换 conda 环境。
- 使用 6.2 节中对应平台的可见设备变量——变量写错会导致任务静默地只跑单卡。
